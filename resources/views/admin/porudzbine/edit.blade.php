...
<body>
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-12">
                <h2>Izmeni Porudžbinu</h2>
                
                <form action="{{ route('admin.porudzbine.update', $porudzbina->PorudzbinaID) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="KupacID" name="KupacID" required>
                            @foreach($kupci as $kupac)
                                <option value="{{ $kupac->KupacID }}" 
                                    {{ $porudzbina->KupacID == $kupac->KupacID ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="proizvod_id" name="proizvod_id">
                            ...
                            @foreach($proizvodi as $proizvod)
                                <option value="{{ $proizvod->ProizvodID }}" 
                                    {{ $porudzbina->proizvod_id == $proizvod->ProizvodID ? 'selected' : '' }}>
                                    {{ $proizvod->Naziv }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control" id="Datum_prijave" 
                               name="Datum_prijave" 
                               value="{{ old('Datum_prijave', \Carbon\Carbon::parse($porudzbina->Datum_prijave)->format('Y-m-d\TH:i')) }}" 
                               required>
                        @error('Datum_prijave')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="ZaposleniID" name="ZaposleniID" required>
                            @foreach($zaposleni as $zap)
                                <option value="{{ $zap->ZaposleniID }}" 
                                    {{ $porudzbina->ZaposleniID == $zap->ZaposleniID ? 'selected' : '' }}>
                                    {{ $zap->Ime }} {{ $zap->Prezime }}
                                </option>
                            @endforeach
                        </select>
